        ...
        switch (name) {
            case 'assign_group_id': {
                ...
                try {
                    ...
                    throw new McpError(ErrorCode.InvalidParams, `Failed to assign group ID: ${error instanceof Error ? error.message : String(error)}`);
                }
                ...
                try {
                    ...
                    if (transaction.type === 'appl') {
                        ...
                        if (transaction.appArgs) {
                            transaction.appArgs = transaction.appArgs.map((arg) => new Uint8Array(Buffer.from(arg, 'base64')));
                        }
                    ...
                    throw new McpError(ErrorCode.InvalidParams, `Failed to sign transaction: ${error instanceof Error ? error.message : String(error)}`);
                }
            }
            default:
                throw new McpError(ErrorCode.MethodNotFound, `Unknown general transaction tool: ${name}`);
        }
    }
}
