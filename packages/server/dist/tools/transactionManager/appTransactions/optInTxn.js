import { makeApplicationOptInTxnFromObject, OnApplicationComplete } from 'algosdk';
 ...
 */
export function makeApplicationOptInTxn(params) {
    try {
        ...
        throw new McpError(ErrorCode.InternalError, `Failed to create application opt-in transaction: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
/**
 ...
 */
export function handleOptInTxn(args, suggestedParams) {
    try {
        ...
        throw new McpError(ErrorCode.InternalError, `Failed to handle application opt-in: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
