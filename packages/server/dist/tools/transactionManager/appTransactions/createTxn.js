import { makeApplicationCreateTxnFromObject, OnApplicationComplete } from 'algosdk';
 ...
 */
export function makeApplicationCreateTxn(params) {
    try {
        ...
        throw new McpError(ErrorCode.InternalError, `Failed to create application transaction: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
/**
 ...
 */
export function handleCreateTxn(args, suggestedParams) {
    try {
        ...
        txnParams.approvalProgram = args.approvalProgram;
        txnParams.clearProgram = args.clearProgram;
        ...
        throw new McpError(ErrorCode.InternalError, `Failed to handle application creation: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
