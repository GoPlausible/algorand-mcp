import { makeApplicationDeleteTxnFromObject, OnApplicationComplete } from 'algosdk';
 ...
 */
export function makeApplicationDeleteTxn(params) {
    try {
        ...
        return makeApplicationDeleteTxnFromObject({
            ...txnParams,
            onComplete: OnApplicationComplete.DeleteApplicationOC
        }); // Use type assertion since the types are not perfectly aligned
    }
    catch (error) {
        console.error('[MCP Error] Failed to create application delete transaction:', error);
        throw new McpError(ErrorCode.InternalError, `Failed to create application delete transaction: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
/**
 ...
 */
export function handleDeleteTxn(args, suggestedParams) {
    try {
        ...
        throw new McpError(ErrorCode.InternalError, `Failed to handle application delete: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
