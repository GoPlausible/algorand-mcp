import { makeApplicationNoOpTxnFromObject, OnApplicationComplete } from 'algosdk';
 ...
 */
export function makeApplicationCallTxn(params) {
    try {
        ...
        throw new McpError(ErrorCode.InternalError, `Failed to create application call transaction: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
/**
 ...
 */
export function handleCallTxn(args, suggestedParams) {
    try {
        ...
        throw new McpError(ErrorCode.InternalError, `Failed to handle application call: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
