...
export class AlgodManager {
        ...
        try {
            switch (name) {
                ...
                case 'simulate_transactions':
                    ...
                    const simulateResult = await AlgodManager.simulateTransactions(args);
                    return {
                        content: [{
                                type: 'text',
    ...
    static async compile(source) {
        try {
            ...
            throw new McpError(ErrorCode.InternalError, `Failed to compile TEAL: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
     ...
     */
    static async disassemble(bytecode) {
        try {
            const response = await algodClient.disassemble(bytecode).do();
            return response;
        }
        catch (error) {
            console.error('[MCP Error] Failed to disassemble TEAL:', error);
            throw new McpError(ErrorCode.InternalError, `Failed to disassemble TEAL: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
     ...
     */
    static async sendRawTransaction(signedTxns) {
        try {
            ...
            throw new McpError(ErrorCode.InternalError, `Failed to send transaction: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
     ...
     */
    static async simulateRawTransactions(txns) {
        try {
            const response = await algodClient.simulateRawTransactions(txns).do();
            return response;
        }
        catch (error) {
            console.error('[MCP Error] Failed to simulate raw transactions:', error);
            throw new McpError(ErrorCode.InternalError, `Failed to simulate raw transactions: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
    ...
    static async simulateTransactions(request) {
        try {
            const simulateRequest = new modelsv2.SimulateRequest({
                txnGroups: request.txnGroups.map(group => new modelsv2.SimulateRequestTransactionGroup({ txns: group.txns })),
            ...
            const response = await algodClient.simulateTransactions(simulateRequest).do();
            return response;
        }
        catch (error) {
            console.error('[MCP Error] Failed to simulate transactions:', error);
            throw new McpError(ErrorCode.InternalError, `Failed to simulate transactions: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
}
