...
export class AccountManager {
            ...
            switch (name) {
                ...
                case 'rekey_account':
                    if (!args.sourceAddress || !args.targetAddress ||
                        ...
                        throw new McpError(ErrorCode.InvalidParams, 'Invalid rekey account parameters');
                    }
                    const rekeyTxn = await AccountManager.createRekeyTransaction(args.sourceAddress, args.targetAddress);
                    return {
                        content: [{
                                type: 'text',
    ...
    static createAccount() {
        try {
            ...
            throw new McpError(ErrorCode.InternalError, `Failed to create account: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    /**
     ...
     */
    static async createRekeyTransaction(fromAddress, toAddress) {
        try {
            ...
            throw new McpError(ErrorCode.InternalError, `Failed to create rekey transaction: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    /**
     ...
     */
    static mnemonicToMasterDerivationKey(mnemonic) {
        try {
            return algosdk.mnemonicToMasterDerivationKey(mnemonic);
        }
        catch (error) {
            console.error('[MCP Error] Failed to convert mnemonic to MDK:', error);
            throw new McpError(ErrorCode.InvalidParams, `Invalid mnemonic provided: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    /**
     ...
     */
    static masterDerivationKeyToMnemonic(mdk) {
        try {
            return algosdk.masterDerivationKeyToMnemonic(mdk);
        }
        catch (error) {
            console.error('[MCP Error] Failed to convert MDK to mnemonic:', error);
            throw new McpError(ErrorCode.InvalidParams, `Invalid master derivation key provided: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    /**
     ...
     * @returns The mnemonic
     */
    static secretKeyToMnemonic(secretKey) {
        try {
            return algosdk.secretKeyToMnemonic(secretKey);
        }
        catch (error) {
            console.error('[MCP Error] Failed to convert secret key to mnemonic:', error);
            throw new McpError(ErrorCode.InvalidParams, `Invalid secret key provided: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    /**
     ...
     * @returns The mnemonic
     */
    static mnemonicFromSeed(seed) {
        try {
            return algosdk.mnemonicFromSeed(seed);
        }
        catch (error) {
            console.error('[MCP Error] Failed to generate mnemonic from seed:', error);
            throw new McpError(ErrorCode.InvalidParams, `Invalid seed provided: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
}
