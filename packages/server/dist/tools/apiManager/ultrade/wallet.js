...
async function makeRequest(endpoint, options = {}) {
    ...
    try {
        ...
        if (!response.ok) {
            throw new McpError(ErrorCode.InternalError, `Ultrade API error: ${response.status} ${response.statusText} ${await response.text()}`);
        }
        let result;
...
async function getSigninMessage(data, customMessage) {
    return makeRequest('/wallet/signin/message', {
        method: 'POST',
        body: { data, customMessage }
    });
}
async function signin(message, signature, data, referralToken) {
...
async function getWalletKeys(auth) {
...
async function getWalletTrades(auth) {
    const headers = {
        'x-wallet-address': auth.walletAddress
    };
    if (auth.walletToken)
...
async function getWalletTransactions(auth) {
    const headers = {
        'x-wallet-address': auth.walletAddress
    };
    if (auth.walletToken)
...
async function getWithdrawMessage(data, customMessage) {
...
export async function handleWalletTools(args) {
    switch (args.name) {
