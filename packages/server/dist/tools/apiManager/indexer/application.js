...
export async function lookupApplications(appId) {
    try {
...
export async function lookupApplicationLogs(appId, params) {
    try {
...
export async function lookupApplicationBoxByIDandName(appId, boxName) {
    try {
        let boxNameBytes;
        ...
        const response = await indexerClient.lookupApplicationBoxByIDandName(appId, boxNameBytes).do();
...
export async function searchForApplicationBoxes(appId, maxBoxes) {
    try {
        ...
        throw new McpError(ErrorCode.InternalError, `Failed to get application boxes: ${error instanceof Error ? error.message : String(error)}`);
    }
}
export const handleApplicationTools = ResponseProcessor.wrapResourceHandler(async function handleApplicationTools(args) {
