...
export async function lookupTransactionByID(txId) {
    try {
...
export async function lookupAccountTransactions(address, params) {
    try {
...
export async function searchForTransactions(params) {
    try {
        ...
        throw new McpError(ErrorCode.InternalError, `Failed to search transactions: ${error instanceof Error ? error.message : String(error)}`);
    }
}
export const handleTransactionTools = ResponseProcessor.wrapResourceHandler(async function handleTransactionTools(args) {
