...
export async function lookupAccountByID(address) {
    try {
...
export async function lookupAccountAssets(address, params) {
    try {
...
export async function lookupAccountAppLocalStates(address) {
    try {
        console.log(`Looking up app local states for address ${address}`);
        const response = await indexerClient.lookupAccountAppLocalStates(address).do();
...
export async function lookupAccountCreatedApplications(address) {
    try {
        console.log(`Looking up created applications for address ${address}`);
        const response = await indexerClient.lookupAccountCreatedApplications(address).do();
...
export async function searchAccounts(params) {
    try {
        ...
        throw new McpError(ErrorCode.InternalError, `Failed to search accounts: ${error instanceof Error ? error.message : String(error)}`);
    }
}
export const handleAccountTools = ResponseProcessor.wrapResourceHandler(async function handleAccountTools(args) {
