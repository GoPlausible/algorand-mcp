...
export const handleProtocolTools = ResponseProcessor.wrapResourceHandler(async function handleProtocolTools(args) {
    ...
    try {
        ...
        if (!response.ok) {
            throw new McpError(ErrorCode.InternalError, `Vestige API error: ${response.status} ${response.statusText}`);
        }
        ...
        throw new McpError(ErrorCode.InternalError, `Failed to fetch protocol data: ${error instanceof Error ? error.message : String(error)}`);
    }
});
