...
import { generateOptIntoAssetTxns, generateOptIntoValidatorTxns, generateOptOutOfValidatorTxns } from '@tinymanorg/tinyman-js-sdk';
...
export async function handleOptInTools(args) {
    ...
    try {
        ...
        if (name === 'api_tinyman_get_asset_optin_quote') {
            ...
            throw new McpError(ErrorCode.MethodNotFound, `Unknown opt-in tool: ${name}`);
        }
        return {
            version,
            initiatorAddr,
        ...
        throw new McpError(ErrorCode.InternalError, `Failed to get opt-in/out quote: ${error instanceof Error ? error.message : String(error)}`);
    }
}
