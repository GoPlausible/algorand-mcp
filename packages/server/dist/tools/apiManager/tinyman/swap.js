...
import { Swap, poolUtils, SwapType } from '@tinymanorg/tinyman-js-sdk';
...
export async function handleSwapTools(args) {
    const { name, assetIn, assetOut, amount, mode, version = 'v2' } = args;
    if (name === 'api_tinyman_get_swap_quote') {
        try {
            ...
            if (version === 'v2') {
                ...
                if (mode === 'fixedInput') {
                    quote = Swap.v1_1.getQuote(SwapType.FixedInput, poolInfo, reserves, {
                        id: assetIn,
                        amount: BigInt(amount)
                    }, {
                    ...
                    quote = Swap.v1_1.getQuote(SwapType.FixedOutput, poolInfo, reserves, {
                        id: assetOut,
                        amount: BigInt(amount)
                    }, {
                        assetIn: assetInDecimals,
            ...
            throw new McpError(ErrorCode.InternalError, `Failed to get swap quote: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    throw new McpError(ErrorCode.MethodNotFound, `Unknown swap tool: ${name}`);
}
