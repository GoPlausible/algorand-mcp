...
export async function handlePoolTools(args) {
    ...
    if (name === 'api_tinyman_get_pool') {
        try {
            ...
            throw new McpError(ErrorCode.InternalError, `Failed to get pool information: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    throw new McpError(ErrorCode.MethodNotFound, `Unknown pool tool: ${name}`);
}
