...
import { Bootstrap } from '@tinymanorg/tinyman-js-sdk';
...
export async function handleBootstrapTools(args) {
    const { name, asset1Id, asset2Id, initiatorAddr, version = 'v2' } = args;
    if (name === 'api_tinyman_get_pool_creation_quote') {
        try {
            ...
            throw new McpError(ErrorCode.InternalError, `Failed to get pool creation quote: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    throw new McpError(ErrorCode.MethodNotFound, `Unknown bootstrap tool: ${name}`);
}
