...
import { RemoveLiquidity, poolUtils } from '@tinymanorg/tinyman-js-sdk';
...
export const removeLiquidityTools = [
    {
...
export async function handleRemoveLiquidityTools(args) {
    const { name, asset1Id, asset2Id, poolTokenAmount, initiatorAddr, slippage = 0.01, singleAssetMode = false, version = 'v2' } = args;
    if (name === 'api_tinyman_get_remove_liquidity_quote') {
        try {
            ...
            throw new McpError(ErrorCode.InternalError, `Failed to get remove liquidity quote: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    throw new McpError(ErrorCode.MethodNotFound, `Unknown remove liquidity tool: ${name}`);
}
