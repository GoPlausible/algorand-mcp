...
export async function handleAnalyticsTools(args) {
    const { name, asset1Id, asset2Id, version = 'v2' } = args;
    if (name === 'api_tinyman_get_pool_analytics') {
        try {
            ...
            throw new McpError(ErrorCode.InternalError, `Failed to get pool analytics: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    throw new McpError(ErrorCode.MethodNotFound, `Unknown analytics tool: ${name}`);
}
