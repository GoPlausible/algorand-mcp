...
import { AddLiquidity, poolUtils } from '@tinymanorg/tinyman-js-sdk';
...
export async function handleLiquidityTools(args) {
    const { name, asset1Id, asset2Id, asset1Amount = 0, asset2Amount = 0, mode, version = 'v2' } = args;
    if (name === 'api_tinyman_get_liquidity_quote') {
        try {
            ...
            if (version === 'v2') {
                switch (mode) {
                    ...
                    case 'singleAsset':
                        ...
                        break;
                    default:
                        throw new McpError(ErrorCode.InvalidParams, `Invalid liquidity mode: ${mode}`);
                }
            }
            else {
            ...
            throw new McpError(ErrorCode.InternalError, `Failed to get liquidity quote: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    throw new McpError(ErrorCode.MethodNotFound, `Unknown liquidity tool: ${name}`);
}
