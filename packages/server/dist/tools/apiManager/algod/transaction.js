...
export async function getPendingTransaction(txId) {
    try {
        ...
        throw new McpError(ErrorCode.InternalError, `Failed to get pending transaction: ${error instanceof Error ? error.message : String(error)}`);
    }
}
export async function getPendingTransactionsByAddress(address) {
    try {
        console.log(`Fetching pending transactions for address ${address}`);
        const response = await algodClient.pendingTransactionByAddress(address).do();
        ...
        throw new McpError(ErrorCode.InternalError, `Failed to get pending transactions by address: ${error instanceof Error ? error.message : String(error)}`);
    }
}
export async function getPendingTransactions(maxTxns) {
    try {
        ...
        throw new McpError(ErrorCode.InternalError, `Failed to get pending transactions: ${error instanceof Error ? error.message : String(error)}`);
    }
}
export async function getTransactionParams() {
    try {
        ...
        throw new McpError(ErrorCode.InternalError, `Failed to get transaction params: ${error instanceof Error ? error.message : String(error)}`);
    }
}
export async function getNodeStatus() {
    try {
        ...
        throw new McpError(ErrorCode.InternalError, `Failed to get status: ${error instanceof Error ? error.message : String(error)}`);
    }
}
export async function getNodeStatusAfterBlock(round) {
    try {
        console.log(`Fetching node status after round ${round}`);
        const response = await algodClient.statusAfterBlock(round).do();
        console.log('Node status response:', JSON.stringify(response, null, 2));
        return response;
    }
    catch (error) {
        ...
        throw new McpError(ErrorCode.InternalError, `Failed to get status after block: ${error instanceof Error ? error.message : String(error)}`);
    }
}
export async function handleTransactionTools(name, args) {
    switch (name) {
