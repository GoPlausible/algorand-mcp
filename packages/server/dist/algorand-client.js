...
const buildAlgodUri = (path) => {
    return `${env.algorand_algod_api}${path}`;
};
const buildIndexerUri = (path) => {
...
export const algodClient = new algosdk.Algodv2(env.algorand_token || '', env.algorand_algod, env.algorand_algod_port || '');
// Initialize the Indexer client with base URL
export const indexerClient = new algosdk.Indexer(env.algorand_token || '', env.algorand_indexer, env.algorand_indexer_port || '');
