...
const getAccountFromMnemonic = () => {
    if (!env.algorand_agent_wallet_active) {
        throw new McpError(ErrorCode.InvalidRequest, 'No active wallet mnemonic configured');
    }
...
export const walletResources = {
    ...
    handle: async (uri) => {
        if (!env.algorand_agent_wallet_active) {
            throw new McpError(ErrorCode.InvalidRequest, 'Wallet resources are not available - no active wallet configured');
        }
        switch (uri) {
            case 'algorand://wallet/account':
                try {
                    ...
                    const algodClient = new algosdk.Algodv2(env.algorand_token, env.algorand_algod, env.algorand_algod_port);
            ...
            case 'algorand://wallet/secretkey':
                try {
                    ...
                    throw new McpError(ErrorCode.InternalError, `Failed to get secret key: ${error instanceof Error ? error.message : String(error)}`);
                }
            case 'algorand://wallet/publickey':
                try {
                    ...
                    throw new McpError(ErrorCode.InternalError, `Failed to get public key: ${error instanceof Error ? error.message : String(error)}`);
                }
            case 'algorand://wallet/mnemonic':
                try {
                    ...
                    throw new McpError(ErrorCode.InternalError, `Failed to get mnemonic: ${error instanceof Error ? error.message : String(error)}`);
                }
            case 'algorand://wallet/address':
                try {
                    ...
                    throw new McpError(ErrorCode.InternalError, `Failed to get address: ${error instanceof Error ? error.message : String(error)}`);
                }
            default:
