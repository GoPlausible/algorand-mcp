...
// Load JSON files
const loadJson = (filename) => JSON.parse(fs.readFileSync(path.join(__dirname, 'taxonomy-categories', filename), 'utf-8'));
...
export const knowledgeResources = {
    ...
    handle: async (uri) => {
        ...
        if (categoryMatch) {
            ...
            if (!categoryData) {
                throw new McpError(ErrorCode.InvalidRequest, `Unknown category: ${category}`);
            }
            return {
                contents: [{
                        uri,
        ...
        throw new McpError(ErrorCode.InvalidRequest, `Invalid knowledge URI: ${uri}`);
    },
