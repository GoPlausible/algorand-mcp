...
export async function handleOptInTools(args: any): Promise<any> {
  ...
  try {
    ...
    if (name === 'resource_tinyman_get_asset_optin_quote') {
      const { assetId } = args;
    ...
    else if (name === 'resource_tinyman_get_validator_optin_quote') {
    ...
    else if (name === 'resource_tinyman_get_validator_optout_quote') {
