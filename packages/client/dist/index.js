...
export class AlgorandMcpClient {
    constructor(options) {
        ...
        this.makeTransactionSigner = async (txnGroup, indexesToSign) => {
        ...
        try {
            ...
            switch (walletType) {
                ...
                case 'pera':
                    ...
                    this.activeWallet = this.peraWallet;
                    break;
                case 'defly':
                    accounts = await this.deflyWallet.connect();
                    this.activeWallet = this.deflyWallet;
                    break;
                case 'daffi':
                    accounts = await this.daffiWallet.connect();
                    this.activeWallet = this.daffiWallet;
                    break;
                default:
    ...
    async reconnectSession(walletType) {
        try {
            let accounts = [];
            switch (walletType) {
                case 'local':
